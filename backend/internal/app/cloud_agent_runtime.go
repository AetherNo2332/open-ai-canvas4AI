...
type cloudAgentRuntime struct {
	...
	// readCacheExecution 是上游 2cedc4c6 新增的进程内标记：只有缓存未命中的真读才计入
	// ReadToolCalls 预算（见 cloudAgentReadTool），命中回放不计额度。
	...
	// AutoPreparedMedia / AutoPreparedCallHash 是上游 993580a7 新增的 auto 准入检查点：
	// worker 在 dry admission 与计费提交之间重启时，不重复准入、不重复建草稿节点。
...
func (s *Service) advanceCloudAgentTool(run *model.CloudAgentExecution, state *cloudAgentRuntime) error {
	...
	// 合并取舍：上游 993580a7 把 model_list 并入"技能/目录只读结果"的检查点缓存路径
	// （上面的 skillResult 预读 + 下面 switch 的同一 case），因此撤下 fork 在写事务外
	// 单独预读 modelList 的这条并行实现；模型目录仍由 cloudAgentModelList(userID, intent)
	// 签发 selectionId。
