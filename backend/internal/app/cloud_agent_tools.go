...
func cloudAgentCanonicalFor(system string, history []providerTextMessage, prompt string, req CloudAgentRequest, includeProfileTool bool) canonicalAgentRequest {
	...
	// 合并取舍：缓存键取上游 2cedc4c6 的云 Agent 稳定前缀方案（identity 版本化 +
	// 前缀哈希），与 cloud_agent.go 创建 run 时的写法保持同一签名；fork 上一轮收敛的
	// 一参 cloudAgentPromptCacheKey(req)+sortedCloudAgentIDs 一并撤下（上游是同一机制
	// 的后续版本，保留并行实现会让每次同步都再冲突）。
...
func compileCloudAgentTools(req CloudAgentRequest, includeProfileTool bool) []map[string]any {
	...
	add("finish_run",
		...
		map[string]any{"summary": str("给用户的最终答复全文")},
		"summary")
	if len(req.ContextScope) > 0 {
		...
		// 合并取舍：描述文案取上游 2cedc4c6 的压缩版（工具表是每步都要发的固定开销）；
		// `rows` 是 fork 独有的分镜分页参数，合并后的实现仍走
		// cloudAgentCanvasStatePage(..., readRows)，故保留该参数并在描述里点明用法。
		...
		add("canvas_read_storyboard", "分页读取分镜脚本的结构化镜头行，返回 rowId、snapshotHash。通读用 rows=5（默认5行，每字段≤2000字符）翻页，逐字精读或取单行用 rows=1（每字段≤16000字符）。update/remove 必须使用最新 rowId、snapshotHash，不要猜ID或复制整表。", map[string]any{"nodeId": str("真实分镜脚本节点ID"), "offset": map[string]any{"type": "integer", "minimum": 0}, "rows": map[string]any{"type": "integer", "minimum": 1, "maximum": cloudAgentMaxReadRows, "description": "本页行数：通读用 5，逐字精读用 1"}}, "nodeId")
...
// 合并取舍：上游 2cedc4c6 用「只读边界 + 快照版本化缓存键 + 命中回执」替换了旧的
// cloudAgentMaxCachedReadReplays 重放上限，故该常量随上游撤下（fork 侧已无引用）。
...
func cloudAgentReadTool(repo *repository.Repository, userID string, state *cloudAgentRuntime, call cloudAgentCall, services ...*Service) (any, error) {
	...
	case "model_list":
		...
		// 合并取舍：model_list 取上游 993580a7 的派发位置（只读工具路径），
		// 但目录构造保留 fork 的 userID 参数——selectionId 凭证需要调用者身份才能签发。
