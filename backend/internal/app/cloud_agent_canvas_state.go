...
// 合并取舍：入口保持上游的调用形态（连线偏移可选）；四种读取形态统一走
// cloudAgentCanvasStateSelected，fork 的行级分页从 cloudAgentCanvasStatePage 传 readRows 进来。
func cloudAgentCanvasState(repo *repository.Repository, userID, canvasID string, doc map[string]any, offset int, ids []string, storyboardOffset int, connectionOffsets ...int) (any, error) {
	return cloudAgentCanvasStateSelected(repo, userID, canvasID, doc, offset, ids, nil, 0, false, 0, storyboardOffset, connectionOffsets...)
}
...
func cloudAgentCanvasStateWithFocus(repo *repository.Repository, userID, canvasID string, doc map[string]any, offset int, focusNodeIDs []string, depth, storyboardOffset int, connectionOffsets ...int) (any, error) {
	return cloudAgentCanvasStateSelected(repo, userID, canvasID, doc, offset, nil, focusNodeIDs, depth, false, 0, storyboardOffset, connectionOffsets...)
}
...
func cloudAgentCanvasStateWithRelated(repo *repository.Repository, userID, canvasID string, doc map[string]any, offset int, focusNodeIDs []string, storyboardOffset int, connectionOffsets ...int) (any, error) {
	return cloudAgentCanvasStateSelected(repo, userID, canvasID, doc, offset, nil, focusNodeIDs, 0, true, 0, storyboardOffset, connectionOffsets...)
}

// cloudAgentCanvasStatePage 是并集后的读取入口：上游的「连线分页 + 单页字节预算 + 关联子图」
// 全部保留，另外接受我们自己的结构化行级分页行数（readRows）；四种读取形态最终都走
// cloudAgentCanvasStateSelected，避免同一份投影逻辑出现第二份实现。
func cloudAgentCanvasStatePage(repo *repository.Repository, userID, canvasID string, doc map[string]any, offset int, ids []string, storyboardOffset, connectionOffset, readRows int) (any, error) {
	return cloudAgentCanvasStateSelected(repo, userID, canvasID, doc, offset, ids, nil, 0, false, readRows, storyboardOffset, connectionOffset)
}

func cloudAgentCanvasStateSelected(repo *repository.Repository, userID, canvasID string, doc map[string]any, offset int, ids, focusNodeIDs []string, depth int, includeRelated bool, readRows int, storyboardOffset int, connectionOffsets ...int) (any, error) {
	...
	// 显式按 nodeIds / focusNodeIds 读取是"逐字"路径：只有它返回真实 rowId，
	// 而编辑工具必须用 rowId（fork 的三档投影里对应 Detail 档）。
	...
	for candidateIndex, node := range candidateNodes {
		...
		// fork 的三档投影：逐字路径（nodeIds/focus）用 Detail，其余用 Page。
