...
// 即使这一步拿不到新的实测，也要先把"口径已变/太旧"的锚点作废。
func (s *Service) recordCloudAgentTokenAnchor(state *cloudAgentRuntime, budget cloudAgentContextBudget) {
	if s == nil || state == nil {
		return
	}
	windowTokens := cloudAgentAnchorWindowTokens(budget)
	cloudAgentExpireTokenAnchor(state.RuntimeRunID, state, windowTokens)
